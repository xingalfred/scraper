...
def get_cheapest_prices(
    ...
    cheapest_prices: dict[api.FuelType, api.FuelPrice] = {}
    for prices in prices_list:
        for price in prices.data:
            if price.ean not in cheapest_prices:
                cheapest_prices[price.ean] = price
                continue

            if cheapest_prices[price.ean].price >= price.price:
                cheapest_prices[price.ean] = price

    store_price_list: list[tuple[api.Store, api.FuelPrice]] = []
    for fuel_type in api.FuelType:
        price = cheapest_prices[fuel_type]
